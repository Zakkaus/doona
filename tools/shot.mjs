...
for (let i = 0; i < 20; i++) {
  const r = await send('Runtime.evaluate', {expression: `!!document.querySelector(${JSON.stringify(sel)})`});
  if (r.result?.result?.value) break;
  await new Promise(r => setTimeout(r, 500));
}
if (!touch)
  await send('Runtime.evaluate', {
    expression: `document.documentElement.style.setProperty('--s2-scale','1'); document.documentElement.style.setProperty('--s2-font-size-base','14'); 'ok'`
  });
await new Promise(r => setTimeout(r, 1500));
const box = await send('Runtime.evaluate', {
  expression: `(() => { const r = document.querySelector(${JSON.stringify(sel)}).getBoundingClientRect(); return JSON.stringify({x: r.x, y: r.y, w: r.width, h: r.height}); })()`
});
if (!box.result?.result?.value) {
  console.error(JSON.stringify(box));
  process.exit(1);
}
const b = JSON.parse(box.result.result.value);
const shot = await send('Page.captureScreenshot', {
  format: 'png',
  clip: {x: b.x, y: b.y, width: b.w, height: Math.min(b.h, 1400), scale: 1},
