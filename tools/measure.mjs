...
const scale = 3;
const [asc, desc] = (process.env.OVR || '98,22').split(',');
import {readFileSync} from 'node:fs';
const faces = readFileSync(new URL('../src/fonts.css', import.meta.url), 'utf8')
...
for (const [name, [l0, t0, r0, b0]] of Object.entries(boxes)) {
  const l = Math.round(l0 * scale),
    t = Math.round(t0 * scale),
    r = Math.round(r0 * scale),
    b = Math.round(b0 * scale);
  const bg = name === 'search' ? px(Math.round((l + r) / 2), t + 4) : px(l + 4 * scale, Math.round((t + b) / 2));
  const ink = (x, y) => {
    const p = px(x, y);
    return Math.abs(p[0] - bg[0]) + Math.abs(p[1] - bg[1]) + Math.abs(p[2] - bg[2]) > 150;
  };
  const [xa, xb] = name === 'search' ? [l + 40 * scale, Math.min(r - 40 * scale, l + 260 * scale)] : [l + 12 * scale, r - 12 * scale];
  const rows = [];
  for (let y = t + 3 * scale; y < b - 3 * scale; y++)
  ...
  if (!rows.length) {
    console.log(name.padEnd(12), 'no ink');
    continue;
  }
  const top = (rows[0] - t) / scale,
    bot = (b - rows[rows.length - 1]) / scale;
