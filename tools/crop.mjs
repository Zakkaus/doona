...
const faces = process.env.OVR === 'none' ? readFileSync(new URL('../src/fonts.css', import.meta.url), 'utf8').replace(/(ascent|descent|line-gap)-override: [^;]+; ?/g, '') : readFileSync(new URL('../src/fonts.css', import.meta.url), 'utf8').replace(/ascent-override: \d+%/g, 'ascent-override: ' + asc + '%').replace(/descent-override: \d+%/g, 'descent-override: ' + desc + '%');
...
await send('Page.enable'); await send('Runtime.enable'); await send('Network.enable'); await send('Network.setCacheDisabled', {cacheDisabled: true}); await send('Network.enable'); if (process.env.BLOCK) await send('Network.setBlockedURLs', {urls: ['*typekit.net*']});
...
const r = await send('Runtime.evaluate', {returnByValue: true, expression: `(() => { const want = ${JSON.stringify(want)}; const out = {}; const seen = {};
...
for (const [name, b] of Object.entries(r.result.result.value)) {
  const shot = await send('Page.captureScreenshot', {format: 'png', clip: {x: b.x, y: b.y, width: b.w, height: b.h, scale: 4 / +(process.env.DPR || 1)}, captureBeyondViewport: true});
  const f = dir + '/' + name.replace(/[#:\[\]=\s]/g, '-') + '.png'; writeFileSync(f, Buffer.from(shot.result.data, 'base64')); console.log(f, JSON.stringify(b));
}
