...
這個區塊請放在獨立的 include（`include { api.dae }`）：主檔若含 `native_api.secret`，API 不會回傳它的內容，也不允許寫入，組態頁就無法編輯主檔。
