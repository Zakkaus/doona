...
發行檔（`doona-<version>.tar.gz`、選用的 `doona-fonts-<version>.tar.gz`（Noto Sans TC 與 SC）、`SHA256SUMS`）附在[發行頁](https://github.com/Zakkaus/doona/releases)的標籤上。自行建置請見[開發](#開發)一節。驗證檔案並解壓至引擎或 Web 伺服器提供檔案的目錄：

```sh
VERSION=0.1.0-beta.7  # 替換為下載檔案對應的版本，不含 v
