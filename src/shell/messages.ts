import {defineMessages} from '../i18n';

export const messages = defineMessages({
  'zh-TW': {
    'conn.chain': '鏈',
    'conn.rule': '規則',
    'conn.recomputed': '重算',
    'nav.routingTrace': '路由追蹤',
    'nav.compat': '示範',
    'lifecycle.starting': '啟動中',
    'lifecycle.running': '運行中',
    'lifecycle.reloading': '重載中',
    'lifecycle.suspended': '已暫停',
    'lifecycle.draining': '排空中',
    'lifecycle.degraded': '降級',
    'lifecycle.failed': '失敗',
    'nav.activity': '活動',
    'nav.overview': '概覽',
    'nav.connections': '連線',
    'nav.clients': '客戶端',
    'nav.policies': '策略',
    'nav.dns': 'DNS',
    'nav.resources': '資源',
    'nav.config': '配置',
    'nav.validate': '校驗',
    'nav.events': '事件',
    'nav.flows': '流程',
    'grp.status': '狀態',
    'grp.network': '網路',
    'grp.proxy': '代理',
    'grp.system': '系統',
    search: '搜尋連線、節點、群組與頁面',
    refresh: '重新讀取',
    'theme.system': '跟隨系統',
    'theme.light': '亮',
    'theme.dark': '暗',
    palette: '配色',
    'palette.glass': '毛玻璃',
    wordmark: '字標',
    'wordmark.gradient': '漸層',
    'wordmark.plain': '純色',
    lang: '語言',
    page: '頁面',
    github: 'GitHub 倉庫',
    'search.nodes': '節點',
    'search.none': '沒有符合的項目',
    'search.groups': '群組',
    'search.pages': '頁面',
    'shell.backendRequired': '此示範頁面僅適用於 Clash 相容模式。',
    'conn.state.observed': '觀測中',
    'conn.state.routing': '路由中',
    'conn.state.dialing': '撥號中',
    'conn.state.active': '進行中',
    'conn.state.closed': '已關閉',
    'conn.state.blocked': '已封鎖',
    'conn.state.failed': '失敗',
    'conn.state.unknown': '未知',
    'shell.theme': '主題：{theme}',
    'shell.shortcut': 'Ctrl K',
    'shell.macShortcut': '⌘K',
    'palette.rosePine': 'Rosé Pine',
    'palette.moon': 'Moon',
    'palette.dawnMain': 'Dawn / Main',
    'palette.dawnMoon': 'Dawn / Moon',
    'palette.catppuccin': 'Catppuccin',
    'palette.frappe': 'Frappé',
    'palette.macchiato': 'Macchiato',
    'palette.mocha': 'Mocha',
    'palette.latteFrappe': 'Latte / Frappé',
    'palette.latteMacchiato': 'Latte / Macchiato',
    'palette.latteMocha': 'Latte / Mocha',
    'palette.nord': 'Nord',
    'palette.nordVariants': 'Snow Storm / Polar Night',
    'palette.antd': 'Ant Design',
    'palette.defaultDark': '預設／暗色',
    'palette.bytedance': 'ByteDance',
    'palette.arco': 'Arco Design',
    'palette.semi': 'Semi Design',
    'palette.lightDark': '亮色／暗色',
    'palette.glassName': '玻璃'
  },
  'zh-CN': {
    'conn.chain': '链',
    'conn.rule': '规则',
    'conn.recomputed': '重算',
    'nav.routingTrace': '路由追踪',
    'nav.compat': '演示',
    'lifecycle.starting': '启动中',
    'lifecycle.running': '运行中',
    'lifecycle.reloading': '重载中',
    'lifecycle.suspended': '已暂停',
    'lifecycle.draining': '排空中',
    'lifecycle.degraded': '降级',
    'lifecycle.failed': '失败',
    'nav.activity': '活动',
    'nav.overview': '概览',
    'nav.connections': '连接',
    'nav.clients': '客户端',
    'nav.policies': '策略',
    'nav.dns': 'DNS',
    'nav.resources': '资源',
    'nav.config': '配置',
    'nav.validate': '校验',
    'nav.events': '事件',
    'nav.flows': '流程',
    'grp.status': '状态',
    'grp.network': '网络',
    'grp.proxy': '代理',
    'grp.system': '系统',
    search: '搜索连接、节点、组和页面',
    refresh: '重新读取',
    'theme.system': '跟随系统',
    'theme.light': '亮',
    'theme.dark': '暗',
    palette: '配色',
    'palette.glass': '毛玻璃',
    wordmark: '字标',
    'wordmark.gradient': '渐变',
    'wordmark.plain': '纯色',
    lang: '语言',
    page: '页面',
    github: 'GitHub 仓库',
    'search.nodes': '节点',
    'search.none': '没有符合的项目',
    'search.groups': '组',
    'search.pages': '页面',
    'shell.backendRequired': '此演示页面仅适用于 Clash 兼容模式。',
    'conn.state.observed': '观测中',
    'conn.state.routing': '路由中',
    'conn.state.dialing': '拨号中',
    'conn.state.active': '进行中',
    'conn.state.closed': '已关闭',
    'conn.state.blocked': '已阻止',
    'conn.state.failed': '失败',
    'conn.state.unknown': '未知',
    'shell.theme': '主题：{theme}',
    'shell.shortcut': 'Ctrl K',
    'shell.macShortcut': '⌘K',
    'palette.rosePine': 'Rosé Pine',
    'palette.moon': 'Moon',
    'palette.dawnMain': 'Dawn / Main',
    'palette.dawnMoon': 'Dawn / Moon',
    'palette.catppuccin': 'Catppuccin',
    'palette.frappe': 'Frappé',
    'palette.macchiato': 'Macchiato',
    'palette.mocha': 'Mocha',
    'palette.latteFrappe': 'Latte / Frappé',
    'palette.latteMacchiato': 'Latte / Macchiato',
    'palette.latteMocha': 'Latte / Mocha',
    'palette.nord': 'Nord',
    'palette.nordVariants': 'Snow Storm / Polar Night',
    'palette.antd': 'Ant Design',
    'palette.defaultDark': '默认／深色',
    'palette.bytedance': 'ByteDance',
    'palette.arco': 'Arco Design',
    'palette.semi': 'Semi Design',
    'palette.lightDark': '浅色／深色',
    'palette.glassName': '玻璃'
  },
  en: {
    'conn.chain': 'Chain',
    'conn.rule': 'Rule',
    'conn.recomputed': 'Recomputed',
    'nav.routingTrace': 'Routing trace',
    'nav.compat': 'Demo',
    'lifecycle.starting': 'Starting',
    'lifecycle.running': 'Running',
    'lifecycle.reloading': 'Reloading',
    'lifecycle.suspended': 'Suspended',
    'lifecycle.draining': 'Draining',
    'lifecycle.degraded': 'Degraded',
    'lifecycle.failed': 'Failed',
    'nav.activity': 'Activity',
    'nav.overview': 'Overview',
    'nav.connections': 'Connections',
    'nav.clients': 'Clients',
    'nav.policies': 'Policies',
    'nav.dns': 'DNS',
    'nav.resources': 'Resources',
    'nav.config': 'Config',
    'nav.validate': 'Validate',
    'nav.events': 'Events',
    'nav.flows': 'Flows',
    'grp.status': 'Status',
    'grp.network': 'Network',
    'grp.proxy': 'Proxy',
    'grp.system': 'System',
    search: 'Search connections, nodes, groups and pages',
    refresh: 'Reload data',
    'theme.system': 'System',
    'theme.light': 'Light',
    'theme.dark': 'Dark',
    palette: 'Palette',
    'palette.glass': 'Frosted glass',
    wordmark: 'Wordmark',
    'wordmark.gradient': 'Gradient',
    'wordmark.plain': 'Plain',
    lang: 'Language',
    page: 'Page',
    github: 'GitHub repository',
    'search.nodes': 'Nodes',
    'search.none': 'No matches',
    'search.groups': 'Groups',
    'search.pages': 'Pages',
    'shell.backendRequired': 'This demo page is only available in Clash-compatible mode.',
    'conn.state.observed': 'Observed',
    'conn.state.routing': 'Routing',
    'conn.state.dialing': 'Dialing',
    'conn.state.active': 'Active',
    'conn.state.closed': 'Closed',
    'conn.state.blocked': 'Blocked',
    'conn.state.failed': 'Failed',
    'conn.state.unknown': 'Unknown',
    'shell.theme': 'Theme: {theme}',
    'shell.shortcut': 'Ctrl K',
    'shell.macShortcut': '⌘K',
    'palette.rosePine': 'Rosé Pine',
    'palette.moon': 'Moon',
    'palette.dawnMain': 'Dawn / Main',
    'palette.dawnMoon': 'Dawn / Moon',
    'palette.catppuccin': 'Catppuccin',
    'palette.frappe': 'Frappé',
    'palette.macchiato': 'Macchiato',
    'palette.mocha': 'Mocha',
    'palette.latteFrappe': 'Latte / Frappé',
    'palette.latteMacchiato': 'Latte / Macchiato',
    'palette.latteMocha': 'Latte / Mocha',
    'palette.nord': 'Nord',
    'palette.nordVariants': 'Snow Storm / Polar Night',
    'palette.antd': 'Ant Design',
    'palette.defaultDark': 'Default / Dark',
    'palette.bytedance': 'ByteDance',
    'palette.arco': 'Arco Design',
    'palette.semi': 'Semi Design',
    'palette.lightDark': 'Light / Dark',
    'palette.glassName': 'Glass'
  }
});
