...
export const messages = defineMessages({
  'zh-TW': {
    ...
    'ruleDialog.added': '已新增規則，重新載入完成（r{revision}）',
    'ruleDialog.addReload': '新增並重新載入',
    'ruleDialog.basis': '依據',
    'ruleDialog.beforeFallback': 'fallback 之前（config.dae:44）',
    'ruleDialog.beforeHit': '命中的規則之前',
    'ruleDialog.rulesEnd': 'rules.dae 結尾',
    'ruleDialog.must': '強制規則，全域與直連模式下仍然生效'
  },
  'zh-CN': {
    'conn.chain': '链',
    'conn.rule': '规则',
    ...
    'palette.glassName': '玻璃',
    'ruleDialog.added': '已新增规则，重新加载完成（r{revision}）',
    'ruleDialog.addReload': '新增并重新加载',
    'ruleDialog.basis': '依据',
    'ruleDialog.beforeFallback': 'fallback 之前（config.dae:44）',
    'ruleDialog.beforeHit': '匹配的规则之前',
    'ruleDialog.rulesEnd': 'rules.dae 末尾',
    'ruleDialog.must': '强制规则，在全局与直连模式下仍然生效'
  },
  en: {
    'conn.chain': 'Chain',
    'conn.rule': 'Rule',
    ...
    'ruleDialog.added': 'Rule added and reloaded (r{revision})',
    'ruleDialog.addReload': 'Add and reload',
    'ruleDialog.basis': 'Based on',
    'ruleDialog.beforeFallback': 'Before fallback (config.dae:44)',
    'ruleDialog.beforeHit': 'Before the matched rule',
    'ruleDialog.rulesEnd': 'End of rules.dae',
    'ruleDialog.must': 'Required rule: also applies in global and direct modes'
  }
});
