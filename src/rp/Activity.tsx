...
export function Activity({go}: {go: (page: string) => void}) {
  ...
  const issues: Issue[] = [
    ...timeoutIssues(NODES.filter(n => n.unavailable).map(n => n.name), t('act.timeout'), t('act.nTimeouts'), t('act.sep')).map(text => ({level: 'err' as const, text, page: 'policies'})),
  ...
  return (
    <>
      <div className="rp-quick">
        ...
        <div className="rp-card"><div className="rp-row"><Light tone={liveRuntime.lifecycle.state === 'running' ? 'ok' : 'warn'}>{t(`lifecycle.${liveRuntime.lifecycle.state}` as 'lifecycle.running')}</Light><Button quiet onPress={() => go('overview')}>{t('act.viewDetails')}</Button></div></div>
      </div>
      ...
      <div className="rp-g3">
        ...
        <div className="rp-card">
          ...
          <div className="rp-list">{[...NODES].sort((x, y) => (x.alive ? x.tcp ?? 0 : 1e9) - (y.alive ? y.tcp ?? 0 : 1e9)).slice(0, 6).map(n => <Bar key={n.name} icon={<Flag name={n.name} />} label={n.name} value={n.alive && n.tcp !== undefined ? n.tcp + ' ms' : n.unavailable ? t('act.timeout') : t('act.unknown')} pct={n.alive ? ((n.tcp ?? 0) / 250) * 100 : 100} color={n.alive ? ((n.tcp ?? 0) < 100 ? p.foam : p.gold) : p.love} />)}</div>
        </div>
        <div className="rp-card">
