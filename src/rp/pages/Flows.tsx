...
export function Flows({go, query}: PageProps) {
  ...
  return <div className="rp-page">
    {resource.error && <p role="alert" className="rp-note">無法載入流程：{resource.error.message}</p>}
    {resource.loading && !resource.data && <p role="status">載入中…</p>}
    <div className="rp-toolbar">
      <Segmented label="網路協定" value={network} onChange={setNetwork} items={[["all", '全部'], ['tcp', 'TCP'], ['udp', 'UDP']]} />
      <LabeledSelect label="狀態" side value={state} onChange={setState} items={[{id: 'all', label: '全部狀態'}, ...Object.entries(connectionStates).map(([id, label]) => ({id, label}))]} />
    </div>
    ...
    {flow && <section className="rp-card" aria-label="流程記錄">
      <div className="rp-row"><h3 className="rp-h3">{flow.id}</h3><Badge>{flow.trace.status}</Badge><span className="rp-label">修訂 {flow.revision}</span></div>
      <p className="rp-note">{flow.trace.status === 'partial' ? '流程記錄不完整，未記錄的階段無法還原。' : flow.trace.status === 'disabled' ? '流程追蹤已停用，沒有階段記錄。' : '流程記錄完整。'}</p>
      <Kv items={[["missing", flow.trace.missing.join('、') || '—'], ['連線', flow.connection_id ?? '—'], ['狀態', connectionStates[flow.state] ?? flow.state]]} />
      <ol className="rp-flow-timeline">{[...flow.trace.steps].sort((a, b) => a.seq - b.seq).map(step => {
        const fields = flowStepFields(step);
        return <li key={step.seq} className="rp-flow-step">
          <div className="rp-toolbar"><Badge>{step.stage}</Badge><span className="rp-code">seq {step.seq}</span><time dateTime={step.observed_at ?? undefined}>{step.observed_at ?? '—'}</time><span className="rp-label">{step.elapsed_us ?? '—'} μs</span></div>
          {fields ? <Kv items={fields} /> : <pre className="rp-flow-raw"><code>{JSON.stringify(step.data, null, 2)}</code></pre>}
        </li>;
      })}</ol>
    </section>}
  </div>;
