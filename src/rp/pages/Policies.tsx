...
const LABELS = {
  ...
  byName: '依名稱',
  aliveOnly: '只看可用',
  count: (n: number, down: number) => n + ' 個成員' + (down ? '，' + down + ' 個無法使用' : ''),
  none: '沒有符合的節點'
};
function PolicyCard({
  id,
        ...
        <>
          <div className="rp-row">
            <span className="rp-cluster">
              ...
              <span className="rp-label">{members.length} 個成員</span>
            </span>
            <Button
              small
              isDisabled={!!control.busy || !g.capabilities.probe_transports.includes('tcp')}
              tip={!g.capabilities.probe_transports.includes('tcp') ? '此群組不支援 TCP 探測' : undefined}
              onPress={() => {
...
export function Policies({go}: PageProps) {
  ...
  return (
    <div className="rp-page">
      <div className="rp-toolbar">
        <p className="rp-note">僅支援手動選擇的 selector 群組可切換成員。TCP 與 UDP 可分別選擇。</p>
