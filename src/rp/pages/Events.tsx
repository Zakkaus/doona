      ...
      <Tabs label="事件與日誌" tabs={[['events', '事件'], ['clash', 'Clash 日誌']]}>{id => id === 'events' ? (
        <div className="rp-page">
          <div className="rp-toolbar">
            ...
            <span className="rp-label">重放範圍為行程內最近 200 條，遺漏處會標出；篩選不改變 global.log_level。</span>
          </div>
          <DataTable label="事件" height={342} rows={shown} empty="沒有符合的事件"
            ...
            render={e => [<span className="rp-code">{e.t}</span>, e.kind, <Light small tone={TONE[e.level]}>{e.level}</Light>, t(`ev.${e.id}` as 'ev.e1'), e.ref ? <Button quiet small onPress={() => go(e.ref!.replace('#/', ''))}><OpenIn />開啟</Button> : '']} />
        </div>
      ) : (
