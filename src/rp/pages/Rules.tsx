...
import {runtime} from '../../app/mock';
import {Button, DataTable, Kv, LabeledSelect, TextField, ModalDialog, toast} from '../ui';
...
export function Rules({go}: PageProps) {
  ...
  return (
    <div className="rp-page">
      ...
      <form className="rp-card" onSubmit={e => { e.preventDefault(); void trace.submit(); }}>
        <div className="rp-trace-form">
          <LabeledSelect label="網路協定" value={form.network} onChange={network => setForm({...form, network: network as 'tcp' | 'udp'})} items={[{id: 'tcp', label: 'TCP'}, {id: 'udp', label: 'UDP'}]} />
          ...
          <LabeledSelect label="解析模式" value={form.resolve} onChange={resolve => setForm({...form, resolve: resolve as 'none' | 'live'})} items={trace.modes.map(id => ({id, label: id === 'none' ? '不解析（none）' : '即時解析（live）'}))} />
        </div>
        {trace.invalid && <p className="rp-note">{trace.invalid}</p>}
        {!trace.available && <p className="rp-note">不支援路由追蹤</p>}
        <Button accent isDisabled={trace.busy || !!trace.invalid || !trace.available || !trace.modes.includes(form.resolve)} onPress={() => void trace.submit()}>{trace.busy ? '追蹤中' : '追蹤'}</Button>
      </form>
      {trace.error && <p role="alert">{trace.error.message}</p>}
      {trace.result && <section className="rp-col" aria-label="模擬結果">
        ...
        {trace.result.evaluations.map((evaluation, i) => <section className="rp-card" key={i}>
          ...
          <DataTable label={'規則評估 ' + (i + 1)} height={400} rows={evaluation.rules.map(rule => ({...rule, id: rule.rule_id}))}
            cols={[{id: 'id', label: '規則 ID', width: 90, isRowHeader: true}, {id: 'expression', label: '運算式'}, {id: 'result', label: '結果', width: 180}, {id: 'missing', label: '缺少輸入', width: 150}]}
            render={rule => [rule.rule_id, <span className="rp-code">{rule.expression ?? '—'}</span>, <span className={'rp-light rp-trace-result ' + rule.result}>{rule.result}</span>, rule.missing_inputs.join('、') || '—']} />
        </section>)}
        {trace.result.dns.map(dns => <section className="rp-card" key={dns.lookup_id}>
      ...
      {config && <section className="rp-col">
        <h2 className="rp-h3">配置中的規則（讀自配置檔，非 API）</h2>
        <div className="rp-split">
          <div className="rp-list">
            <DataTable label="配置規則" rows={config.rules} selected={sel} onSelect={setSel}
              cols={[{id: 'n', label: '#', width: 56, align: 'end'}, {id: 'c', label: '條件', isRowHeader: true}, {id: 'o', label: '目標', width: 88}, {id: 'm', label: 'must', width: 80}, {id: 's', label: '來源', width: 130}, {id: 'x', label: '註解', width: 80}]}
              render={r => [r.n, <span className="rp-code">{r.cond}</span>, r.target, r.must ? 'must' : '', r.generated ? '生成' : r.source, r.note]} />
            <div className="rp-fb"><span className="rp-kw">fallback</span>: <span className="rp-out">{config.fallback.target}</span><span className="rp-label">{config.fallback.source}，請在來源編輯器修改</span></div>
          </div>
              ...
              <div className="rp-col">
                <div className="rp-group-btns">
                  ...
                  <ModalDialog alert trigger={<Button negative isDisabled={!cur.editable}>刪除</Button>} title={'刪除規則 #' + cur.n} narrow footer={close => <><Button secondary onPress={close}>取消</Button><Button negative onPress={() => { close(); toast('positive', '已刪除 #' + cur.n + '，reload 完成（r' + (runtime.diskRevision + 1) + '）'); }}>刪除並 reload</Button></>}>
                    <p className="rp-note">從 {cur.source} 刪除 {cur.cond} -&gt; {cur.target}，以 If-Match r{runtime.diskRevision} 寫入，校驗通過後 reload。註解與其他行原樣保留。</p>
                  </ModalDialog>
                </div>
