...
import {Button, LabeledSelect, ModalDialog, Switch, TextField, toast} from './ui';
...
export function RuleDialog({trigger, presets}: {trigger: ReactElement, presets: Preset[]}) {
  ...
  return (
    <ModalDialog trigger={trigger} title="加規則" narrow footer={close => <><Button onPress={close}>取消</Button><Button accent onPress={() => { close(); toast('positive', '已加入規則，reload 完成（r' + (runtime.diskRevision + 1) + '）'); }}>加入並 reload</Button></>}>
      <div className="rp-form">
        ...
        <LabeledSelect label="位置" value={pos} onChange={setPos} items={[{id: 'before-fallback', label: 'fallback 之前（config.dae:44）'}, {id: 'before-hit', label: '命中的規則之前'}, {id: 'rules-end', label: 'rules.dae 結尾'}]} />
