...
const NAV: Array<[string, Array<[string, string, typeof Home]>]> = [
  [
    'grp.status',
    [
      ['activity', 'nav.activity', GraphTrend],
  ...
  [
    'grp.network',
    [
      ['connections', 'nav.connections', Link],
      ['flows', 'nav.flows', ListBulleted],
      ['clients', 'nav.clients', Devices]
    ]
  ],
  [
    'grp.proxy',
    [
      ['policies', 'nav.policies', Share],
      ['rules', 'nav.routingTrace', ListBulleted],
      ['dns', 'nav.dns', Globe]
    ]
  ],
  [
    'grp.system',
    [
      ['resources', 'nav.resources', Data],
      ['config', 'nav.config', FileText],
      ['validate', 'nav.validate', CheckmarkCircle],
      ['events', 'nav.events', History]
    ]
  ]
];
...
function SchemeIcon({dark}: {dark: boolean}) {
  return (
    <span className="rp-icon-stack" data-dark={dark || undefined}>
      <Contrast UNSAFE_className="moon" />
      <Lighten UNSAFE_className="sun" />
    </span>
  );
}
        ...
        {nav.map(([g, items]) => (
          <div key={g} data-group={g.replace('grp.', '')}>
            ...
            {items.map(([k, label, Icon]) => (
              <RLink key={k} className="rp-nav" href={'#/' + k} aria-current={route === k ? 'page' : undefined}>
                <Icon />
                {t(label as 'nav.activity')}
                {compatRoutes[k] && <span className="rp-badge rp-nav-compat">{t('nav.compat')}</span>}
              </RLink>
            ))}
          </div>
        ))}
            ...
            <div className="rp-mobile-nav">
              <LabeledSelect
                ...
                items={nav
                  ...
                  .map(([k, label]) => ({
                    id: k,
                    label: t(label as 'nav.activity'),
                    icon: compatRoutes[k] ? <span className="rp-badge rp-nav-compat">{t('nav.compat')}</span> : undefined
                  }))}
                bare
              />
            </div>
