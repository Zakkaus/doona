...
it('reports every refused main-source write with one validation message and one failure summary', () => {
  expect(editProblem({kind: 'invalid', errors: 2}, t)).toEqual({kind: 'negative', text: 'Validation found 2 errors; nothing written'});
  expect(editProblem({kind: 'failed', error: new Error('offline')}, t)).toEqual({
    kind: 'negative',
    text: 'Could not write the configuration',
    detail: 'offline'
  });
  // An unknown outcome is not a failure, so it gets neither the failure summary nor the failure tone.
  ...
  // A file written but not applied is not a failed write, so it does not get the write-failure summary.
