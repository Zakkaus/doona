...
export function Card({title, note, aside, children}: {title: string; note?: string; aside?: ReactNode; children: ReactNode}) {
  const titleId = useId();
  const noteId = useId();
  const heading = (
  ...
  return (
    <section className="rp-card rp-titled" aria-labelledby={titleId}>
      {aside ? (
