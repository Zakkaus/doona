...
export function BarRow({label, value, pct, color}: {label: string, value: string, pct: number, color: SeriesColor}) {
  return <div className={barRow}><div className={barTop}><span className={barLabel}>{label}</span><span className={barVal}>{value}</span></div><div className={track}><div className={fill({color})} style={{width: `${Math.max(0, Math.min(100, pct))}%`}} /></div></div>;
}
