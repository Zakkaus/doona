...
const swatchClass: Record<SeriesColor, string> = {
  accent: style({backgroundColor: 'yellow-600'}),
  accent2: style({backgroundColor: 'yellow-400'}),
  orange: style({backgroundColor: 'gray-700'}),
  green: style({backgroundColor: 'seafoam-600'}),
  purple: style({backgroundColor: 'indigo-700'}),
  red: style({backgroundColor: 'red-700'}),
  warn: style({backgroundColor: 'orange-600'}),
  dark: style({backgroundColor: 'gray-800'}),
  muted: style({backgroundColor: 'gray-500'}),
  muted2: style({backgroundColor: 'gray-400'})
};
...
export function usePalette<T extends HTMLElement>() {
  ...
  useEffect(() => {
    ...
    mo.observe(document.documentElement, {attributes: true, subtree: true, attributeFilter: ['data-color-scheme', 'style', 'class']});
...
export function Legend({series, fmt}: {series: Series[]; fmt: (v: number | null | undefined) => string}) {
  return (
    <div className={legendRow}>
      {series.map(s => (
        <span key={s.label} className={legendItem}>
          <i className={swatch + ' ' + swatchClass[s.color]} />
          {s.label} <span className={legendVal}>{fmt(s.values[s.values.length - 1])}</span>
        </span>
      ))}
    </div>
  );
    ...
    <div ref={ref} className={box} style={{height}}>
      <ResponsiveContainer width="100%" height="100%">
        <RAreaChart data={data} margin={{top: 8, right: 0, bottom: 0, left: 20}}>
          <defs>
            {series.map((s, k) => (
              <linearGradient key={s.label} id={uid + k} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={p[s.color]} stopOpacity={0.4} />
                <stop offset="100%" stopColor={p[s.color]} stopOpacity={0.04} />
              </linearGradient>
            ))}
          </defs>
          ...
          />
          <Tooltip
            contentStyle={tipStyle(p)}
            itemStyle={{color: p.tipText}}
            labelFormatter={value => new Date(Number(value)).toLocaleString()}
            formatter={v => fmt(Number(v))}
            cursor={{stroke: p.text, strokeDasharray: '3 3'}}
          />
          {series.map((s, k) => (
            <Area
              key={s.label}
...
export function Donut({
  ...
  rows: Array<{name: string; value: number | null; text: string; color: SeriesColor}>;
  total: string;
  caption?: string;
}) {
  const [p, ref] = usePalette<HTMLDivElement>();
      ...
      <div className={donutBox}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart margin={{top: 0, right: 0, bottom: 0, left: 0}}>
            ...
            >
              {data.map(r => (
                <Cell key={r.name} fill={p[r.color]} />
              ))}
            </Pie>
            <Tooltip
              contentStyle={tipStyle(p)}
        ...
        <div className={donutCenter}>
          ...
          {caption && <span className={donutSub}>{caption}</span>}
        </div>
      </div>
      <div className={donutList}>
        {rows.map(r => (
          <div key={r.name} className={donutRow}>
            <i className={swatch + ' ' + swatchClass[r.color]} />
            <span className={donutName}>{r.name}</span>
            <span className={donutBytes}>{r.text}</span>
            <span className={donutPct}>{r.value === null ? '—' : r.value + '%'}</span>
          </div>
        ))}
      </div>
    </div>
...
export function BarRow({label, value, pct, color, icon}: {label: string; value: string; pct: number; color: SeriesColor; icon?: ReactNode}) {
  return (
    <div className={barRow}>
      ...
      <div className={track}>
        <div className={fill({color})} style={{width: `${Math.max(0, Math.min(100, pct))}%`}} />
      </div>
    </div>
  );
}
