...
export const card = style({backgroundColor: 'gray-75', borderRadius: 'lg', padding: 12, boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: 8, minWidth: 0});
...
export const facts = style({display: 'grid', gridTemplateColumns: {default: ['repeat(2, minmax(0, 1fr))'], lg: ['repeat(3, minmax(0, 1fr))'], xl: ['repeat(5, minmax(0, 1fr))']}, columnGap: 16, rowGap: 24});
...
export const frameHead = style({display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, minHeight: 40, paddingX: 16, backgroundColor: 'gray-75', borderBottomWidth: 1, borderStyle: 'solid', borderColor: 'gray-300', font: 'detail'});
export const frameBody = style({overflow: 'auto', paddingY: 12});
export const lineRow = style({display: 'grid', gridTemplateColumns: [56, 'max-content'], font: 'code', whiteSpace: 'pre', paddingEnd: 16, minHeight: 24, alignItems: 'center'});
...
export function Big({v, u}: {v: string, u?: string}) {
  return <div className={style({display: 'flex', alignItems: 'baseline'})}><span className={big}>{v}</span>{u && <span className={unit}>{u}</span>}</div>;
}
export function Kv({items}: {items: Array<[string, string]>}) {
  return <div className={kv}>{items.map(([k, v]) => <LabeledValue key={k} label={k} value={v} />)}</div>;
}
...
export function Ready({ok, children}: {ok: boolean, children: string}) {
  return <StatusLight variant={ok ? 'positive' : 'negative'} size="S"><Text>{children}</Text></StatusLight>;
}
export function Frame({title, actions, children}: {title: ReactNode, actions?: ReactNode, children: ReactNode}) {
  return <div className={frame}><div className={frameHead}><span>{title}</span>{actions}</div><div className={frameBody}>{children}</div></div>;
}
export function Line({n, err, children}: {n: number, err?: boolean, children: ReactNode}) {
  return <div className={lineRow + (err ? ' ' + lineErr : '')}><span className={lineNum}>{n}</span>{children}</div>;
}
...
export function DaeLine({text}: {text: string}) {
  ...
  if (sec && sec[2] === 'fallback') { const t = sec[3].match(/^: (\w+)$/); return <span>{sec[1]}<span className={kw}>fallback</span>: <span className={out}>{t?.[1]}</span></span>; }
  if (sec) return <span>{sec[1]}<span className={kw}>{sec[2]}</span>{sec[3]}</span>;
  const r = text.match(/^(\s*)(.+?) -> (\w+)(\(must\))?$/);
  if (r) return <span>{r[1]}<span className={arg}>{r[2]}</span> -&gt; <span className={out}>{r[3]}{r[4]}</span></span>;
  const s = text.match(/^(\s*)([\w-]+): ('.*')$/);
  if (s) return <span>{s[1]}{s[2]}: <span className={str}>{s[3]}</span></span>;
