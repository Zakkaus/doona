...
export function SearchTrigger({
  ...
  placeholder: string;
  labels: {all: string; conns: string; nodes: string; rules: string; none: string};
  go: (route: string, query?: string) => void;
  compact?: boolean;
}) {
  ...
  const pick = (k: Key) => {
    const [kind, v] = String(k).split(':');
    setOpen(false);
    go(kind === 'rule' ? 'rules' : kind === 'node' ? 'policies' : 'connections', kind === 'conn' ? 'q=' + encodeURIComponent(v) : undefined);
  };
  return (
    <DialogTrigger
      isOpen={open}
    ...
    >
      {compact ? (
        <ActionButton isQuiet aria-label={placeholder}>
          <Search />
        </ActionButton>
      ) : (
      ...
      <CustomDialog size="L" isDismissible>
        <div className={body}>
          ...
          <div className={results + ' thin-scroll'}>
            {total === 0 ? (
              <div className={empty}>{labels.none}</div>
            ) : (
              <Menu aria-label={placeholder} onAction={pick}>
                {found.conns.length > 0 && (
                  <MenuSection>
                    <Header>
                      <Heading>{labels.conns}</Heading>
                    </Header>
                    {found.conns.map(c => (
                      <MenuItem key={c.id} id={'conn:' + (c.host || c.dst)} textValue={c.host || c.dst}>
                        <Text slot="label">{c.host || c.dst}</Text>
                ...
                {found.nodes.length > 0 && (
                  <MenuSection>
                    <Header>
                      <Heading>{labels.nodes}</Heading>
                    </Header>
                    {found.nodes.map(n => (
                      <MenuItem key={n} id={'node:' + n} textValue={n}>
                        {n}
                      </MenuItem>
                    ))}
                  </MenuSection>
                ...
                {found.rules.length > 0 && (
                  <MenuSection>
                    <Header>
                      <Heading>{labels.rules}</Heading>
                    </Header>
                    {found.rules.map(r => (
                      <MenuItem key={r.id} id={'rule:' + r.id} textValue={r.cond}>
                        <Text slot="label">{r.cond}</Text>
                        <Text slot="description">
                          {r.target}
                          {r.must ? '(must)' : ''}
                        </Text>
