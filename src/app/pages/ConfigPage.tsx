      ...
      <div className={split}>
        <div className={col}>
          <div className={between}>
            <div className={row}>
              ...
              <div className={inline}>
                <Badge variant="neutral" fillStyle="outline" size="S">
                  <Text>磁碟 r{runtime.diskRevision}</Text>
                </Badge>
              </div>
              <div className={inline}>
                <Badge variant="neutral" fillStyle="outline" size="S">
                  <Text>執行中 r{runtime.activeRevision}</Text>
                </Badge>
              </div>
              {dirty && (
                <div className={inline}>
            ...
            <ButtonGroup>
              <Button variant="secondary" onPress={() => go('validate')}>
                {errs.length ? `校驗 (${errs.length})` : '校驗'}
              </Button>
              <Button variant="primary" onPress={() => toast('neutral', 'reload 完成（r' + runtime.diskRevision + '）')}>
                reload
              </Button>
              <Button
                variant="accent"
          ...
          {errs.map(e => {
            ...
            return (
              <InlineAlert key={e.line} variant="negative" fillStyle="border">
                <Heading>
                  第 {e.line} 行：{h}
                </Heading>
                {b ? <Content>{b}</Content> : <Content>{e.why}</Content>}
              </InlineAlert>
            );
          })}
          {edit ? (
