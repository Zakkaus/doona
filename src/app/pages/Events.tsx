  ...
  return (
    <div className={page}>
      <Tabs aria-label="事件與日誌">
        ...
        <TabPanel id="events">
          <div className={panel}>
            <div className={toolbar}>
              <Picker aria-label="種類" selectedKey={kind} onSelectionChange={key => key != null && setKind(String(key))}>{[<PickerItem key="all" id="all">所有種類</PickerItem>, ...eventKinds.map(kind => <PickerItem key={kind} id={kind}>{kind}</PickerItem>)]}</Picker>
              <StatusLight variant={feed.connected ? 'positive' : 'notice'} size="S"><Text>{feed.available === false ? '不支援事件串流' : feed.connected ? '已連線' : '重新連線中'}</Text></StatusLight>
            ...
            <TableView aria-label="事件" styles={style({height: 442})}>
              <TableHeader><Column id="t" width={220}>時間</Column><Column id="k" width={190}>種類</Column><Column id="m" isRowHeader>摘要</Column></TableHeader>
              <TableBody items={shown} renderEmptyState={() => '沒有符合的事件'}>
                {event => <Row id={event.id}><Cell><span className={code}>{event.data.observed_at}</span></Cell><Cell>{event.event}</Cell><Cell>{eventSummary(event)}</Cell></Row>}
              </TableBody>
            </TableView>
          </div>
        </TabPanel>
        <TabPanel id="clash">
          <div className={panel}>
            <Frame title="Clash 日誌，最近 4 條" actions={<span className={label}>info 級，跟隨 global.log_level</span>}>{clashLog.map((line, i) => <Line key={i} n={i + 1}><LogLine text={line} /></Line>)}</Frame>
          </div>
        </TabPanel>
      </Tabs>
    </div>
