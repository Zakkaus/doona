  ...
  return (
    <div className={page}>
      ...
      <TableView aria-label="客戶端" styles={style({height: 300})}>
        <TableHeader>
          <Column id="ip" isRowHeader width={150}>
            來源 IP
          </Column>
          <Column id="active" width={100}>
            活動連線
          </Column>
          <Column id="download" width={110}>
            下載合計
          </Column>
          <Column id="outbound">出站</Column>
          <Column id="first" width={190}>
            首次見到
          </Column>
          <Column id="act" width={230}>
            操作
          </Column>
        </TableHeader>
        <TableBody items={clients.rows} renderEmptyState={() => (clients.loading ? '載入中' : '目前沒有客戶端')}>
          {c => (
            <Row id={c.ip}>
              <Cell>
                <span className={code}>{c.ip}</span>
              </Cell>
              <Cell>{c.active}</Cell>
              <Cell>{formatBytes(c.download)}</Cell>
              <Cell>{c.outbounds || '—'}</Cell>
              <Cell>
                <span title={c.firstSeen}>{localTime(c.firstSeen)}</span>
              </Cell>
              <Cell>
                <ActionButtonGroup size="S">
                  <ActionButton onPress={() => go('connections', 'src=' + encodeURIComponent(c.ip))}>
                    <LinkIcon />
                    <Text>連線</Text>
                  </ActionButton>
                  <RuleDialog
