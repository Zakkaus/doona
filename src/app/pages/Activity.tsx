...
function useNodeSections(nodes: LatencyNode[], selectedName: string) {
  const all = useMemo(
    () =>
      nodes.length > 12
        ? [...nodes]
            .sort((a, b) => (a.alive ? (a.tcp ?? 0) : 1e9) - (b.alive ? (b.tcp ?? 0) : 1e9))
            .reduce(
              (acc, n) => {
                const region = regionOf(n.name) ?? '—';
                const section = acc.find(([key]) => key === region);
                if (section) section[1].push(n);
                else acc.push([region, [n]]);
                return acc;
              },
              [] as Array<[string, LatencyNode[]]>
            )
        : [],
    [nodes]
  );
  ...
  const loadMore = () => {
    if (state === 'idle' && shown.length < all.length) setState('loadingMore');
  };
...
const quick = style({
  display: 'grid',
  gridTemplateColumns: {default: ['1fr'], sm: ['repeat(2, minmax(0, 1fr))'], xl: ['minmax(0, 1.5fr)', 'repeat(2, minmax(0, 1fr))']},
  gap: 12
});
...
function More({onPress, children}: {onPress: () => void; children: string}) {
  return (
    <ActionButton isQuiet size="S" onPress={onPress}>
      <Text>{children}</Text>
    </ActionButton>
  );
}
...
export function Activity({go}: PageProps) {
  ...
  const OUT = usage.rows.map((r, i) => ({
    ...
    color: r.name === 'block' ? ('red' as const) : colors[i % colors.length]
  }));
  const sep = t('act.sep2');
  const sep2 = t('lang') === 'Language' ? ': ' : '：';
  const traffic = [
    {label: t('act.download'), color: 'accent' as const, values: series.down},
    {label: t('act.upload'), color: 'orange' as const, values: series.up}
  ];
  ...
  const issues: Issue[] = [
    ...timeoutIssues(
      ...
      t('act.nTimeouts'),
      t('act.sep')
    ).map(text => ({level: 'negative' as const, text, page: 'policies'})),
    ...failing.map(c => ({
    ...
    ...events
      ...
      .map(e => ({
        level: e.level === 'warn' ? ('notice' as const) : ('informative' as const),
  ...
  return (
    <div className={page}>
      <div className={quick}>
        <div className={card}>
          <div className={row}>
            ...
            <SegmentedControl
              ...
              onSelectionChange={k => {
                setMode(k as Mode);
                toast('positive', t('act.mode') + sep2 + t(`mode.${String(k)}` as 'mode.rule'));
              }}
            >
              <SegmentedControlItem id="rule">{t('mode.rule')}</SegmentedControlItem>
              <SegmentedControlItem id="global">{t('mode.global')}</SegmentedControlItem>
              <SegmentedControlItem id="direct">{t('mode.direct')}</SegmentedControlItem>
            </SegmentedControl>
          </div>
        </div>
        <div className={card}>
          <div className={row}>
            ...
            <MenuTrigger>
              ...
              <Menu
                ...
                onSelectionChange={k => {
                  if (k === 'all') return;
                  const next = [...k].find(x => x !== target);
                  if (next) setTarget(String(next));
                }}
              >
                {groups.map(g => (
                  <MenuItem key={g.name} id={g.name}>
                    {g.name}
                  </MenuItem>
                ))}
              </Menu>
        ...
        <div className={card}>
          <div className={row}>
            <StatusLight variant={liveRuntime.lifecycle.state === 'running' ? 'positive' : 'notice'}>
              <Text>{t(`lifecycle.${liveRuntime.lifecycle.state}` as 'lifecycle.running')}</Text>
            </StatusLight>
            <ActionButton isQuiet onPress={() => go('overview')}>
              <Text>{t('act.viewDetails')}</Text>
            </ActionButton>
          </div>
        </div>
      </div>
      ...
      <div className={strip}>
        <Panel>
          <span className={tileHead}>
            <Download styles={grayIcon} />
            {t('act.download')}
          </span>
          <div className={tileBody}>
            <span className={tileVal}>
              <span className={big}>{formatRate(liveRuntime.traffic.rates?.download_bytes_per_second ?? null)}</span>
            </span>
            <span className={tileSpark}>
              <Spark values={series.down} timestamps={series.timestamps} color="accent" />
            </span>
          </div>
        </Panel>
        <Panel>
          <span className={tileHead}>
            <Upload styles={grayIcon} />
            {t('act.upload')}
          </span>
          <div className={tileBody}>
            <span className={tileVal}>
              <span className={big}>{formatRate(liveRuntime.traffic.rates?.upload_bytes_per_second ?? null)}</span>
            </span>
            <span className={tileSpark}>
              <Spark values={series.up} timestamps={series.timestamps} color="orange" />
            </span>
          </div>
        </Panel>
        <Panel>
          <span className={tileHead}>
            <LinkIcon styles={grayIcon} />
            {t('act.active')}
          </span>
          <div className={tileBody}>
            <span className={tileVal}>
              <span className={big}>{liveRuntime.traffic.connections.total ?? '—'}</span>
            </span>
            <span className={tileSpark}>
              <Spark values={series.connections} timestamps={series.timestamps} color="orange" />
            </span>
          </div>
        </Panel>
        <Panel>
          <span className={tileHead}>
            <Clock styles={grayIcon} />
            {t('act.latency')}
            <Picker
              aria-label={t('act.node')}
              isQuiet
              size="S"
              selectedKey={nodeName}
              onSelectionChange={k => {
                if (k != null) setNodeName(String(k));
              }}
              loadingState={sec.loadingState}
              onLoadMore={sec.loadMore}
              renderValue={items => {
                const name = (items[0] as (typeof NODES)[number] | undefined)?.name ?? nodeName;
                return (
            ...
            >
              {sec.grouped
                ? sec.sections.map(([r, list]) => (
                    <PickerSection key={r} id={r}>
                      <Header>
                        <Heading>{r}</Heading>
                      </Header>
                      {list.map(n => (
                        <PickerItem key={n.name} id={n.name} textValue={n.name}>
                          <Text slot="label">
                            <Flag name={n.name} />
                            {n.name}
                          </Text>
                          <Text slot="description">{n.alive && n.tcp !== undefined ? n.tcp + ' ms' : n.unavailable ? t('act.timeout') : t('act.unknown')}</Text>
                        </PickerItem>
                      ))}
                    </PickerSection>
                  ))
          ...
          <div className={tileBody}>
            <span className={tileVal}>
              <span className={big}>{node?.alive ? (node.tcp === undefined ? '—' : node.tcp + ' ms') : '—'}</span>
              {node?.alive && <span className={delta}>↓ 12%</span>}
            </span>
            <StatusLight variant={node?.alive ? 'positive' : 'negative'} size="S">
              <Text>{node?.alive ? t('act.good') : node?.unavailable ? t('act.timeout') : t('act.unknown')}</Text>
            </StatusLight>
          </div>
        </Panel>
      </div>
      ...
      <div className={grid21}>
        <Panel>
          <div className={row}>
            <span className={title}>{t('act.traffic')}</span>
            <div className={scroller}>
              <SegmentedControl aria-label={t('act.historyRange')} selectedKey={range} onSelectionChange={setRange}>
                <SegmentedControlItem id="live">{t('act.live')}</SegmentedControlItem>
                <SegmentedControlItem id="h1">{t('act.h1')}</SegmentedControlItem>
                <SegmentedControlItem id="h6">{t('act.h6')}</SegmentedControlItem>
                <SegmentedControlItem id="h24">{t('act.h24')}</SegmentedControlItem>
                <SegmentedControlItem id="d7">{t('act.d7')}</SegmentedControlItem>
              </SegmentedControl>
            </div>
          </div>
          {history.error ? (
            ...
            <span className={label}>{t('act.noHistory')}</span>
          ) : !history.data ? (
            <span role="status">{t('act.loading')}</span>
          ) : !history.data.samples.length ? (
            <span className={label}>{t('act.emptyHistory')}</span>
          ) : (
            <>
        ...
        <Panel>
          <div className={row}>
            <span className={cluster}>
              <span className={title}>{t('act.outUsage')}</span>
              {outbounds.data && <span className={label}>{t('act.since').replace('{t}', localTime(outbounds.data.counter_since))}</span>}
            </span>
          </div>
          {outbounds.error ? (
            <p role="alert">{outbounds.error.message}</p>
          ) : capabilities.data?.resources.runtime_outbounds.available === false ? (
            <span className={label}>{t('act.noOutbounds')}</span>
          ) : !outbounds.data ? (
            <span role="status">{t('act.loading')}</span>
          ) : (
      ...
      <div className={grid3}>
        <Panel>
          <div className={row}>
            <span className={title}>{t('act.topDevices')}</span>
            <SegmentedControl aria-label={t('act.topDevices')} selectedKey={by} onSelectionChange={setBy}>
              <SegmentedControlItem id="dev">{t('act.devices')}</SegmentedControlItem>
              <SegmentedControlItem id="host">{t('act.domains')}</SegmentedControlItem>
            </SegmentedControl>
          </div>
          <div className={list}>
            {TOP[String(by)].map(([k, v, tx, kind]) => (
              <div key={k} className={devRow}>
                {kind === 'phone' ? <DevicePhone styles={grayIcon} /> : <DeviceDesktop styles={grayIcon} />}
                <BarRow label={k} value={tx + sep + v + '%'} pct={v} color="orange" />
              </div>
            ))}
          </div>
        </Panel>
        <Panel>
          <div className={row}>
            <span className={title}>{t('act.probeLatency')}</span>
            <More onPress={() => go('policies')}>{t('act.viewAll')}</More>
          </div>
          <div className={list}>
            {[...NODES]
              ...
              .map(n => (
                <BarRow
                  ...
                  color={n.alive ? ((n.tcp ?? 0) < 100 ? 'green' : 'warn') : 'red'}
                />
              ))}
          </div>
        </Panel>
        <Panel>
          <div className={row}>
            <span className={cluster}>
              <span className={title}>{t('act.issues')}</span>
              {issues.length > 0 && <span className={label}>{issues.length}</span>}
            </span>
            <More onPress={() => go('overview')}>{t('act.viewAll')}</More>
          </div>
          {issues.length === 0 ? (
            <span className={label}>{t('act.noIssues')}</span>
          ) : (
            <div>
              {issues.map((i, idx) => (
                <div key={i.text}>
                  {idx > 0 && <Divider size="S" />}
                  <div className={issueRow}>
                    <span className={issueLevel}>
                      <StatusLight variant={i.level} size="S">
                        <Text>{i.level === 'negative' ? t('act.lvlErr') : i.level === 'notice' ? t('act.lvlWarn') : t('act.lvlInfo')}</Text>
                      </StatusLight>
                    </span>
                    <span className={issueText}>{i.text}</span>
                  </div>
                </div>
              ))}
            </div>
