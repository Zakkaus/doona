        ...
        {cur ? (
          <div className={card}>
            ...
            <div className={col}>
              ...
              {cur.chain[0] === proxy.name && <Picker label={proxy.name + ' 的選擇（只影響後續撥號）'} selectedKey={proxy.selected} onSelectionChange={k => toast('positive', proxy.name + ' 改選 ' + String(k) + '，已持久化')}>{proxy.members.map(m => <PickerItem key={m} id={m} textValue={m}><Text slot="label">{flagOf(m) && <span className={flagStyle}>{flagOf(m)}</span>}{m}</Text></PickerItem>)}</Picker>}
