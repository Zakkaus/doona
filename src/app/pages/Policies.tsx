...
const member = style({
  ...
  backgroundColor: {default: 'gray-100', isSelected: 'blue-100'},
  font: 'ui',
  color: {default: 'neutral', isSelected: 'blue-1000'},
  textAlign: 'center',
...
function GroupNodes({
  ...
  const members = big
    ? g.members.filter(
        m =>
          (!needle || m.name.toLowerCase().includes(needle) || (regionOf(m.name) ?? '').toLowerCase() === needle) &&
          (!aliveOnly || health.get(m.id)?.state === 'healthy')
      )
    : g.members;
      ...
      {big && (
        <div className={row}>
          <SearchField
            ...
            size="S"
          />
          <Switch isSelected={aliveOnly} onChange={setAliveOnly} size="S">
            只看可用
          </Switch>
          <span className={label}>
            {members.length} 個成員{down ? '，' + down + ' 個無法使用' : ''}
          </span>
        </div>
      )}
      <div className={grid}>
        {members.slice(0, big ? shown : undefined).map(m => {
          ...
          const sub =
            m.kind === 'group' ? (
              <span className={badge}>群組</span>
            ) : h?.state === 'unavailable' ? (
              '無法使用'
            ) : h?.state === 'healthy' && h.latency_ms !== null ? (
              h.transport.toUpperCase() + ' ' + h.latency_ms + ' ms'
            ) : (
              '未知'
            );
          const body = (
            <>
              ...
              <span className={nodeSub}>{sub}</span>
            </>
          );
          return pick ? (
            <ToggleButton
            ...
            <div key={m.id} className={member({isSelected: selected === m.id})}>
              {body}
            </div>
          );
        })}
      </div>
      {big && members.length > shown && (
        <div className={row}>
          <ActionButton size="S" onPress={() => setShown(shown + PAGE)}>
            <Text>顯示更多（剩餘 {members.length - shown} 個）</Text>
          </ActionButton>
        </div>
      )}
    </>
...
function PolicyCard({
  ...
  return (
    <section className={card} aria-label={g?.name ?? id}>
      {control.error && (
        <p role="alert" className={note}>
          無法讀取或更新群組：{control.error.message}
        </p>
      )}
      {!g ? (
        <p role="status">{id} 載入中…</p>
      ) : (
        <>
          <div className={cardHead}>
            <div className={row}>
              <h3 className={h3}>{g.name}</h3>
              <span className={badge}>{g.policy.kind}</span>
              <span className={label}>{g.members.length} 個成員</span>
            </div>
            <TooltipTrigger isDisabled={g.capabilities.probe_transports.includes('tcp')}>
              <ActionButton
                size="S"
                isDisabled={!!control.busy || !g.capabilities.probe_transports.includes('tcp')}
                onPress={() => {
                  void control.probe().then(result => {
                    if (result) toast('positive', g.name + '：' + probeSummary(result));
                  });
                }}
              >
                <Refresh />
                <Text>{control.busy === 'probe' ? '測試中…' : '測試全部'}</Text>
              </ActionButton>
              <Tooltip>此群組不支援 TCP 探測</Tooltip>
            </TooltipTrigger>
          </div>
          <Kv
            items={[
              ['TCP 選擇', tcp ?? '—'],
              ['UDP 選擇', udp ?? '—'],
              ['健康狀態', healthy + ' 個可用，' + unavailable + ' 個無法使用'],
              ['配置修訂', g.config_revision]
            ]}
          />
          <details>
            <summary>配置</summary>
          ...
          <div className={row}>
            {selectable && (
              <SegmentedControl
                aria-label={g.name + ' 選擇網路'}
                selectedKey={control.network}
                onSelectionChange={value => {
                  if (value === 'both' || value === 'tcp' || value === 'udp') control.setNetwork(value);
                }}
              >
                <SegmentedControlItem id="both">兩者</SegmentedControlItem>
              ...
              <Switch
                ...
                onChange={value => {
                  void control.setInterrupt(value).then(saved => {
                    if (saved) toast('positive', g.name + ' 配置已更新');
                  });
                }}
              >
                切換時中斷現有連線
                ...
                ? memberId => {
                    void control.select(memberId).then(result => {
                      if (result)
                        toast(
                          'positive',
                          g.name + ' 已選擇 ' + result.member_id + '；' + (result.connections_interrupted ? '現有連線已中斷' : '現有連線未中斷')
                        );
                    });
                  }
                : undefined
...
export function Policies({go}: PageProps) {
  ...
  return (
    <div className={page}>
      <div className={row}>
        <p className={note}>僅支援手動選擇的 selector 群組可切換成員。TCP 與 UDP 可分別選擇。</p>
        <ActionButton onPress={() => go('resources')}>
          <Data />
          <Text>訂閱來源</Text>
        </ActionButton>
      </div>
      {(groups.error || nodes.error) && (
        <p role="alert" className={note}>
          無法載入策略：{(groups.error ?? nodes.error)?.message}
        </p>
      )}
