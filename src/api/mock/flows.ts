...
export function flowFields(input: FlowDetail['input'], steps: FlowStep[]): FlowFields {
  ...
  return {
    ...
    ingress: input.ingress, domain_source: input.domain_source
  };
}
...
export function createFlow(connection: ConnectionSeed, network: 'tcp' | 'udp', observedAt: string, instanceId: string): FlowDetail {
  ...
  const expression = blocked ? 'domain(suffix: doubleclick.net)' : direct ? 'dip(geoip:cn)' : connection.domain ? 'domain(full: ' + connection.domain + ')' : 'fallback: ' + connection.outbound;
  ...
  if (!direct && !blocked) {
    steps.push({...common, seq: steps.length + 1, stage: 'dial_mode', data: {configured: input.domain ? 'domain' : 'ip', effective_target: input.domain ? 'domain' : 'ip', domain: input.domain, domain_source: input.domain_source, verification: input.domain ? 'matched' : 'not_required', reason: input.domain ? 'dns_mapping_matches' : 'ip_target'}});
    if (input.domain) steps.push({...common, seq: steps.length + 1, stage: 'dns', data: {lookup_id: 'lookup-1', parent_lookup_id: null, attempt_id: null, purpose: 'dial_target', name: input.domain, qtype: 'A', source: 'cache', upstream_transport: null, carrier_transport: null, cache: 'hit', cache_entry_id: null, upstream: null, route_evaluation_ids: [], status: 'NOERROR', addresses: [connection.dst!.split(':')[0]], selected_ip: connection.dst!.split(':')[0], error: null}});
    steps.push(
      {...common, seq: steps.length + 1, stage: 'reroute', data: {performed: false, reason: 'no_new_routing_input', from_evaluation_id: 'eval-1', to_evaluation_id: null}},
      {...common, seq: steps.length + 2, stage: 'outbound', data: {attempt_id: 'attempt-1', parent_attempt_id: null, kind: 'leaf', evaluation_id: 'eval-1', routing_source: 'evaluation', routed_outbound: connection.outbound, effective_outbound: connection.outbound, mode_override: 'none', selection_path: [{group_id: connection.outbound!, member_id: leaf, member_name: leaf, policy, reason: policy === 'selector' ? 'runtime_selection' : 'policy_selection', selection: {previous_member_id: null, metric: null, tolerance_ms: null, candidates: [{member_id: leaf, member_name: leaf, leaf_node_id: leaf, leaf_node_name: leaf, eligible: true, sorting_latency_ms: null, score: null, selected: true, reason: 'selected'}]}}], leaf_node_id: leaf, leaf_node_name: leaf, target: input.domain ? input.domain + ':' + connection.dst!.split(':').at(-1) : input.dst, target_kind: input.domain ? 'domain' : 'ip', dial_ip: connection.dst!.split(':')[0], server_addr: '198.51.100.1:443', resolution_location: input.domain ? 'local_dns' : 'original_ip', status: 'succeeded', error: null}}
    );
  }
