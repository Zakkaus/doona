...
export const connectionStates: Record<Connection['state'], string> = {
  observed: '觀測中',
  routing: '路由中',
  dialing: '撥號中',
  active: '進行中',
...
export function connectionDetails(c: Connection): Array<[string, string]> {
  return [
    ...
    ['域名', c.domain ?? '—'],
    ['入口', c.ingress ?? '—'],
    ['域名來源', c.domain_source ?? '—'],
    ['程序名稱', c.pname ?? '—'],
    ['觀測來源', c.observed_by],
    ['上傳', formatBytes(c.upload_bytes)],
    ['下載', formatBytes(c.download_bytes)],
    ['上傳速率', formatRate(c.upload_bytes_per_second)],
    ['下載速率', formatRate(c.download_bytes_per_second)],
    ['開始時間', c.started_at ?? '—']
  ];
}
...
export function flowStepFields(step: FlowStep): Array<[string, string]> | null {
  ...
  switch (step.stage) {
    case 'input':
      return Object.entries(step.data.values).map(([key, value]) => [key, text(value)]);
    case 'route':
      return [
        ['chain', step.data.chain],
        ['plane', step.data.plane],
        [
          '規則',
          step.data.rules
        ...
        ['outbound', text(step.data.outbound)],
        ['must', text(step.data.must)]
      ];
    case 'dial_mode':
      return [
        ['撥號目標', step.data.configured + ' → ' + step.data.effective_target],
        ['verification', step.data.verification]
      ];
    case 'dns':
      return [
        ['name', step.data.name],
        ['source / cache', step.data.source + ' / ' + step.data.cache],
        ['upstream', text(step.data.upstream)],
        ['selected_ip', text(step.data.selected_ip)]
      ];
    case 'outbound':
      return [
        ['出站', text(step.data.routed_outbound) + ' → ' + text(step.data.effective_outbound)],
        ['選擇路徑', step.data.selection_path.map(p => p.group_id + ' → ' + text(p.member_name ?? p.member_id)).join('；') || '—'],
        ['leaf_node_id', text(step.data.leaf_node_id)],
        ['target', text(step.data.target)],
        ['status', step.data.status]
      ];
    case 'connection':
      return [
        ['狀態', connectionStates[step.data.state] ?? step.data.state],
        ['milestone', step.data.milestone],
    ...
    case 'reroute':
      return [
        ['performed', text(step.data.performed)],
        ['reason', step.data.reason]
      ];
    default:
      return null;
...
export function probeSummary(result: ProbeResult): string {
  ...
  return (
    states.filter(s => s === 'healthy').length +
    ' 個可用，' +
    states.filter(s => s === 'unavailable').length +
    ' 個無法使用' +
    (unknown ? '，' + unknown + ' 個狀態未知' : '') +
    '；' +
    (result.selection_changed.tcp || result.selection_changed.udp ? '選擇已變更' : '選擇未變更')
  );
}
...
export function datapathFields(datapath: Datapath, unknown: string, label: LabelFn = key => key): Array<[string, string]> {
  ...
  return [
    [label('kind'), datapath.kind],
    [label('state'), datapath.state],
    [label('visibility'), datapath.visibility],
    ...(ebpf
      ? ([
          [label('backend'), ebpf.backend],
          [label('programs'), ebpf.programs],
          [label('hooks'), ebpf.hooks],
          [label('routing'), ebpf.routing.state + ' / ' + (ebpf.routing.generation_id ?? '—')],
          [label('health'), ebpf.health],
          [label('lastError'), ebpf.last_error ?? '—'],
...
export function eventSummary(event: ApiEvent): string {
  switch (event.event) {
    case 'stream.ready':
      return event.data.instance_id;
    case 'runtime.updated':
      return event.data.href;
    case 'flow.updated':
      return event.data.resource_id + ' / revision ' + event.data.revision;
    case 'operation.updated':
      return event.data.resource_id + ' / ' + event.data.status;
    case 'generation.changed':
      return event.data.previous_generation_id + ' → ' + event.data.generation_id;
    case 'flow.gap':
      return (event.data.resource_id ?? '—') + ' / ' + event.data.reason + ' / dropped_records ' + (event.data.dropped_records ?? '—');
  }
}
