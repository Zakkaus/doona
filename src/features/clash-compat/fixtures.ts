...
export const checks: Check[] = [
  ...
  {id: 'tproxy', name: '透明監聽', requested: '0.0.0.0:12345', effective: '0.0.0.0:12345', ready: true, detail: 'TCP/UDP 監聽中'},
  {
    ...
    effective: '未掛',
    ready: false,
    detail: 'nfnetlink_queue 模組未載入；iptables 規則未加',
    restart: true,
    fix: 'modprobe nfnetlink_queue 後重啟 honk',
    manual: 'https://github.com/daeuniverse/honk/wiki/nfqueue'
  },
  ...
  {id: 'api', name: 'Clash API', requested: '127.0.0.1:9090', effective: '127.0.0.1:9090', ready: true, detail: 'secret 為空，只綁本機'},
  {
    ...
    effective: '未啟用',
    ready: false,
    detail: 'external-ui 未設定；doona 目前以 local-controller 方式提供',
    fix: '設定 external-ui 或用 local-controller'
  }
];
...
export const conns: Conn[] = [
  {
    id: '1',
    dst: '149.154.167.220:443',
    host: 'api.telegram.org',
    src: '10.0.0.12',
    mac: 'aa:bb:cc:dd:ee:01',
    out: 'proxy',
    chain: ['proxy', 'hk-01'],
    rule: 'domain(geosite: telegram) -> proxy',
    ruleRef: 'config.dae:42',
    plane: 'userspace',
    up: '84 KB',
    down: '1.2 MB',
    age: '4m 12s',
  ...
  {
    id: '2',
    dst: '120.92.78.14:443',
    host: 'cdn.bilibili.com',
    src: '10.0.0.7',
    mac: 'aa:bb:cc:dd:ee:02',
    out: 'direct',
    chain: ['direct'],
    rule: 'domain(geosite: cn) -> direct',
    ruleRef: 'config.dae:41',
    plane: '內核',
    up: '1.1 MB',
    down: '83 MB',
    age: '12m 03s',
  ...
  {
    id: '4',
    dst: '142.250.66.46:443',
    host: 'doubleclick.net',
    src: '10.0.0.31',
    out: 'block',
    chain: ['block'],
    rule: 'domain(suffix: doubleclick.net) -> block',
    ruleRef: 'config.dae:38',
    plane: '拒絕',
    up: '0',
    down: '0',
    age: '—',
  ...
  },
  {
    ...
    out: 'direct',
    chain: ['direct'],
    rule: 'l4proto(udp) && dport(53) -> direct(must)',
    ruleRef: 'config.dae:39',
    plane: '內核',
    up: '2 KB',
    down: '2 KB',
    age: '1s',
  ...
  },
  {
    ...
    out: 'proxy',
    chain: ['proxy', 'hk-01'],
    rule: 'domain(geosite: discord) -> proxy',
    ruleRef: 'rules.dae:7',
    plane: 'userspace',
    up: '210 KB',
    down: '3.4 MB',
  ...
  {
    id: '7',
    dst: '203.0.113.9:8443',
    src: '10.0.0.20',
    mac: 'aa:bb:cc:dd:ee:20',
    out: 'resilient',
    chain: ['resilient', 'sg-01'],
    rule: 'fallback: resilient',
    ruleRef: 'config.dae:44',
    plane: 'userspace',
    up: '40 KB',
    down: '96 KB',
  ...
  },
  {
    ...
    out: 'direct',
    chain: ['direct'],
    rule: 'dip(geoip: private) -> direct(must)',
    ruleRef: 'config.dae:40',
    plane: '內核',
    up: '1 KB',
    down: '3 KB',
    age: '3s',
...
type Group = {
  name: string;
  policy: 'selector' | 'urltest' | 'loadbalance' | 'fallback' | 'score';
  selected?: string;
  members: string[];
  leaf: string;
  nodes: Node[];
...
export const groups: Group[] = [
  {
    ...
    policy: 'selector',
    selected: 'hk-01',
    members: ['hk-01', 'hk-02', 'sg-01', 'jp-01', 'us-01', 'resilient'],
    leaf: 'hk-01',
    nodes: [
      {name: 'hk-01', tcp: 84, udp: 91, v6: true, source: 'sub-a', alive: true},
      {name: 'hk-02', tcp: 91, udp: 88, v6: true, source: 'sub-a', alive: true},
      {name: 'sg-01', tcp: 63, udp: 70, v6: false, source: 'sub-a', alive: true},
      {name: 'jp-01', v6: false, source: 'sub-b', alive: false},
      {name: 'us-01', tcp: 188, udp: 201, v6: true, source: 'sub-b', alive: true}
    ]
  },
  {
    name: 'resilient',
    policy: 'score',
    members: ['hk-01', 'sg-01', 'us-01'],
    leaf: 'sg-01',
    nodes: [
      {name: 'hk-01', tcp: 84, udp: 91, v6: true, source: 'sub-a', alive: true},
      {name: 'sg-01', tcp: 63, udp: 70, v6: false, source: 'sub-a', alive: true},
...
function bigGroup(count: number): Group {
  ...
  for (let i = 0; i < count; i++) {
    ...
    nodes.push({
      name: region + ' ' + n + (tag ? ' · ' + tag : ''),
      tcp: alive ? tcp : undefined,
      udp: alive ? tcp + Math.round(rnd() * 20) : undefined,
      v6: rnd() > 0.5,
      source: 'sub-c',
      alive
    });
...
const bigCount = (() => {
  try {
    const v = localStorage.getItem('doona-mock-big');
    return v == null ? 100 : Number(v) || 0;
  } catch {
    return 100;
  }
})();
...
type Ev = {
  id: string;
  t: string;
  kind: 'reload' | '訂閱' | '探測' | 'datapath' | 'operation';
