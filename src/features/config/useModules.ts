...
export function useModules({config, editor, canWrite, canValidate}: ModulesProps) {
  ...
  const {clear} = useDraftGuard(dirty);
  ...
  useEffect(() => {
    if (!canValidate || fullText === null || !sourceId) return;
    const controller = new AbortController();
    const timer = setTimeout(() => {
      void api.validateConfig({sources: [candidate({id: sourceId}, fullText)], mode: 'full'}, controller.signal).then(
        result => {
  ...
  }, [api, canValidate, fullText, sourceId]);
  const shown = found ?? (editor.errorSource === sourceId ? editor.diagnostics : null) ?? config.diagnostics;
  ...
  const validate = async () => {
    if (!draft || fullText === null || editor.busy) return;
    const result = await editor.validate({sources: [candidate(draft.section.source, fullText)], mode: 'full'});
