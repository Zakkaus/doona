...
import {ChartCard, MarkerPlot} from '../../ui/charts';
import {latencyGroups, latencyMax, type LatencyBy} from './latency';
...
export function NodeLatency() {
  ...
  const answer = measured.length
    ? t('nodes.latency.answer', {
        fast: measured[0].name,
        fastMs: ms(measured[0].latest),
        slow: measured[measured.length - 1].name,
        slowMs: ms(measured[measured.length - 1].latest),
        down
      })
    : t('nodes.latency.answerNone');
  const dash = (value: number | null) => (value === null ? '—' : ms(value));
  return (
    <ChartCard id="node-latency" question={t('nodes.latency.title')} answer={answer} sample={t('nodes.latency.sample', {n: nodes.data.length})}>
      ...
      <MarkerPlot
        ...
        groups={view.map(group => ({
          ...
          missing: group.missing.map(row => ({
            id: row.id,
            label: row.name,
            text: row.state === 'unavailable' ? t('ui.unavailable') : t('nodes.latency.unmeasured')
          }))
        }))}
      />
    </ChartCard>
