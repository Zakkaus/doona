  ...
  return (
    <div className={page}>
      ...
      <div className={between}>
        <StatusLight variant={state === 'running' ? 'positive' : state === 'failed' ? 'negative' : 'notice'}>
          <Text>{state ? t(`lifecycle.${state}`) : capabilities.loading || runtime.loading ? t('ov.loading') : t('ov.unknown')}</Text>
        </StatusLight>
        <Kv
          items={[
        ...
        <section className={card}>
          ...
          {datapath.data ? (
            <>
              ...
              {datapath.data.ebpf && (
                <TableView aria-label={t('ov.attachments')} styles={style({height: 250})}>
                  <TableHeader>
                    <Column id="n" isRowHeader>
                      {t('ov.name')}
                    </Column>
                    <Column id="i">{t('ov.interface')}</Column>
                    <Column id="d">{t('ov.direction')}</Column>
                    <Column id="s">{t('ov.state')}</Column>
                  </TableHeader>
                  <TableBody renderEmptyState={() => t('ov.unknown')}>
                    {(datapath.data.ebpf.attachments ?? []).map((a, i) => (
                      <Row key={i} id={i}>
                        <Cell>{a.name}</Cell>
                        <Cell>{a.interface}</Cell>
                        <Cell>{a.direction}</Cell>
                        <Cell>{a.state}</Cell>
                      </Row>
                    ))}
                  </TableBody>
          ...
          <section className={card}>
            ...
            <div className={row}>
              {(['reload', 'suspend', 'resume'] as const).map(kind => (
                <Button key={kind} variant="primary" isDisabled={!!operations.busy || !operations.canRun(kind)} onPress={() => void run(kind)}>
                  {t(`ov.${kind}`)}
