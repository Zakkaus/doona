...
const LEVEL: Record<Diag['level'], {variant: 'negative' | 'notice' | 'informative'; label: string}> = {
  error: {variant: 'negative', label: '錯誤'},
  warn: {variant: 'notice', label: '警告'},
  info: {variant: 'informative', label: '提示'}
};
const where = (d: Diag) =>
  (sources
  ...
  return (
    <div className={page}>
      <div className={toolbar}>
        <StatusLight variant={errors ? 'negative' : warns ? 'notice' : 'positive'}>
          <Text>{errors ? `未通過：${errors} 個錯誤、${warns} 個警告` : warns ? `通過，${warns} 個警告` : '通過'}</Text>
        </StatusLight>
        <Badge variant="neutral" fillStyle="subtle">
          磁碟 r{runtime.diskRevision}
        </Badge>
        <Badge variant="neutral" fillStyle="subtle">
          執行中 r{runtime.activeRevision}
        </Badge>
        <span className={grow} />
        <Button variant="primary" onPress={() => toast(errors ? 'negative' : 'positive', errors ? `校驗完成，${errors} 個錯誤` : '校驗通過')}>
          <Refresh />
          <Text>重新校驗</Text>
        </Button>
      </div>
      ...
      <div className={split}>
        <div className={col}>
          <div className={between}>
            <SegmentedControl aria-label="等級" selectedKey={filter} onSelectionChange={setFilter}>
              <SegmentedControlItem id="all">全部 {diagnostics.length}</SegmentedControlItem>
              <SegmentedControlItem id="error">錯誤 {errors}</SegmentedControlItem>
              <SegmentedControlItem id="warn">警告 {warns}</SegmentedControlItem>
              <SegmentedControlItem id="info">提示 {diagnostics.length - errors - warns}</SegmentedControlItem>
            </SegmentedControl>
          </div>
          <TableView
            aria-label="校驗結果"
            ...
            styles={style({height: 360})}
          >
            <TableHeader>
              <Column id="level" width={84}>
                等級
              </Column>
              <Column id="where" width={150}>
                位置
              </Column>
              <Column id="msg" isRowHeader>
                訊息
              </Column>
              <Column id="action" width={88}>
                處理
              </Column>
            </TableHeader>
            <TableBody items={rows} renderEmptyState={() => '沒有這個等級的項目'}>
              {d => (
                <Row id={d.id}>
                  <Cell>
                    <StatusLight variant={LEVEL[d.level].variant} size="S">
                      <Text>{LEVEL[d.level].label}</Text>
                    </StatusLight>
                  </Cell>
                  <Cell>
                    <span className={code}>{where(d)}</span>
                  </Cell>
                  <Cell>{d.msg}</Cell>
                  <Cell>{d.action}</Cell>
                </Row>
        ...
        <div className={col}>
          {cur && (
            <div className={card}>
              <div className={row}>
                <StatusLight variant={LEVEL[cur.level].variant}>
                  <Text>{LEVEL[cur.level].label}</Text>
                </StatusLight>
                <span className={code}>{where(cur)}</span>
              </div>
              <p className={p}>{cur.msg}</p>
              <span className={label}>
                為什麼{cur.action}：{cur.why}。
              </span>
              <Button variant="secondary" onPress={() => open(cur)}>
                開啟來源
              </Button>
            </div>
          )}
          <div className={card}>
            ...
            <div className={list}>
              {pending.map(x => (
                <div key={x.key} className={diff}>
                  <span className={code}>{x.key}</span>
                  <span className={grow}>
                    {x.from} → {x.to}
                  </span>
                  {x.restart && (
