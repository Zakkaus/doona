...
export function Flows({go, query}: PageProps) {
  ...
  const connectionId = q.get('connection_id') ?? undefined;
  const resource = useFlows(connectionId);
  const id = q.get('id') ?? resource.data?.flows.find(f => !connectionId || f.connection_id === connectionId)?.id ?? null;
  ...
  return (
    <div className={page}>
      ...
      <div className={toolbar}>
        <SegmentedControl aria-label="網路協定" selectedKey={network} onSelectionChange={setNetwork}>
          <SegmentedControlItem id="all">全部</SegmentedControlItem>
          <SegmentedControlItem id="tcp">TCP</SegmentedControlItem>
          <SegmentedControlItem id="udp">UDP</SegmentedControlItem>
        </SegmentedControl>
        <Picker label="狀態" labelPosition="side" selectedKey={state} onSelectionChange={k => k != null && setState(k)}>
          <PickerItem id="all">全部狀態</PickerItem>
          {Object.entries(connectionStates).map(([id, text]) => (
      ...
      <div className={toolbar} aria-label="觀測涵蓋範圍">
        {resource.data &&
      ...
      <TableView
        ...
        onSelectionChange={keys => {
          if (keys !== 'all') {
            const value = [...keys][0];
            if (value != null) go('flows', 'id=' + encodeURIComponent(value));
          }
        }}
      ...
      >
        <TableHeader>
          ...
          <Column id="target">目標</Column>
          <Column id="chain" minWidth={150}>
            {t('conn.chain')}
          </Column>
          <Column id="rule" minWidth={180}>
            {t('conn.rule')}
          </Column>
          <Column id="network" width={80}>
            協定
          </Column>
          <Column id="state" width={100}>
            狀態
          </Column>
          <Column id="started" width={100}>
            開始
          </Column>
        </TableHeader>
        <TableBody items={shown} renderEmptyState={() => '沒有符合的流程'}>
          {f => (
            <Row id={f.id}>
              <Cell>{f.id}</Cell>
              <Cell>{f.input?.domain || f.input?.dst || '—'}</Cell>
              <Cell>{chainLabel(f)}</Cell>
              <Cell textValue={f.rule_expression ?? '—'}>
                <span className={ruleCell}>
                  <span className={ruleText} title={f.rule_expression ?? undefined}>
                    {f.rule_expression ?? '—'}
                  </span>
                  {f.rule_source === 'recomputed' ? (
                    <small className={provenance}>{t('conn.recomputed')}</small>
                  ) : f.rule_source === 'unknown' ? (
                    <small className={provenance}>—</small>
                  ) : null}
                </span>
              </Cell>
              <Cell>{f.network.toUpperCase()}</Cell>
              <Cell>{connectionStates[f.state] ?? f.state}</Cell>
              <Cell>{relativeStart(f.started_at)}</Cell>
            </Row>
          )}
        </TableBody>
      </TableView>
      ...
      {flow && (
        <section className={card} aria-label="流程記錄">
          <div className={toolbar}>
            <h3 className={h3}>{flow.id}</h3>
            <span className={badge}>{flow.trace.status}</span>
            <span className={label}>修訂 {flow.revision}</span>
          </div>
          <p className={note}>
          ...
          <ol className={timeline}>
            {[...flow.trace.steps]
              ...
              .map(step => {
                ...
                return (
                  <li key={step.seq} className={stepStyle}>
                    <div className={toolbar}>
                      <span className={badge}>{step.stage}</span>
                      <span className={code}>seq {step.seq}</span>
                      <time dateTime={step.observed_at ?? undefined}>{step.observed_at ?? '—'}</time>
                      <span className={label}>{step.elapsed_us ?? '—'} μs</span>
                    </div>
                    {fields ? (
                      <Kv items={fields} />
                    ) : (
