    ...
    <div className={page}>
      ...
      <div className={toolbar}>
        ...
        <SegmentedControl aria-label="網路協定" selectedKey={network} onSelectionChange={setNetwork}>
          <SegmentedControlItem id="all">全部 {rows.length}</SegmentedControlItem>
          <SegmentedControlItem id="tcp">TCP</SegmentedControlItem>
          <SegmentedControlItem id="udp">UDP</SegmentedControlItem>
        </SegmentedControl>
        <Picker aria-label="出站" selectedKey={out} onSelectionChange={k => k != null && setOut(k)}>
          <PickerItem id="all">所有出站</PickerItem>
          {outbounds.map(id => (
      ...
      <div className={page}>
        <TableView aria-label="連線" selectionMode="single" selectedKeys={sel} onSelectionChange={setSel} styles={style({height: 442})}>
          <TableHeader>
            <Column id="dst" isRowHeader minWidth={132}>
              目標
            </Column>
            <Column id="src" width={136}>
              來源
            </Column>
            <Column id="chain" minWidth={132}>
              {t('conn.chain')}
            </Column>
            <Column id="rule" minWidth={160}>
              {t('conn.rule')}
            </Column>
            <Column id="state" width={96}>
              狀態
            </Column>
            <Column id="down" width={104}>
              下載
            </Column>
            <Column id="age" width={112}>
              開始
            </Column>
          </TableHeader>
          <TableBody items={shown} renderEmptyState={() => '沒有符合的連線'}>
            {c => (
              <Row id={c.id}>
                <Cell textValue={c.domain || c.dst || '—'}>{c.domain || c.dst || '—'}</Cell>
                <Cell>
                  <span className={code}>{c.src ?? '—'}</span>
                </Cell>
                <Cell>{chainLabel(c)}</Cell>
                <Cell textValue={c.rule_expression ?? '—'}>
                  <span className={ruleCell}>
                    <span className={ruleText} title={c.rule_expression ?? undefined}>
                      {c.rule_expression ?? '—'}
                    </span>
                ...
                <Cell>{connectionStates[c.state] ?? c.state}</Cell>
                <Cell>{formatBytes(c.download_bytes)}</Cell>
                <Cell>{relativeStart(c.started_at)}</Cell>
              </Row>
            )}
          </TableBody>
        </TableView>
        {cur ? (
          <div className={card}>
            ...
            <StatusLight
              variant={cur.state === 'blocked' || cur.state === 'failed' ? 'negative' : cur.state === 'active' ? 'positive' : 'informative'}
              size="S"
            >
              <Text>
            ...
            <Button
              variant="secondary"
              onPress={() => go('flows', cur.flow_id ? 'id=' + encodeURIComponent(cur.flow_id) : 'connection_id=' + encodeURIComponent(cur.id))}
            >
              查看流程
            </Button>
            <RuleDialog
              trigger={<Button variant="accent">新增規則</Button>}
              presets={[
          ...
          <div className={card}>
            <span className={label}>選擇連線以查看詳細資料。</span>
          </div>
        )}
      </div>
    </div>
