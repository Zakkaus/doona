...
const formGrid = style({
  display: 'grid',
  gridTemplateColumns: {default: ['minmax(0, 1fr)'], md: ['repeat(2, minmax(0, 1fr))'], xl: ['repeat(4, minmax(0, 1fr))']},
  gap: 16
});
    ...
    <div className={page}>
      ...
      >
        <div className={formGrid}>
          <Picker label="網路協定" selectedKey={form.network} onSelectionChange={k => k != null && setForm({...form, network: k as 'tcp' | 'udp'})}>
            <PickerItem id="tcp">TCP</PickerItem>
            <PickerItem id="udp">UDP</PickerItem>
          </Picker>
          ...
          <Picker label="解析模式" selectedKey={form.resolve} onSelectionChange={k => k != null && setForm({...form, resolve: k as 'none' | 'live'})}>
            {trace.modes.map(id => (
              <PickerItem key={id} id={id}>
                {id === 'none' ? '不解析（none）' : '即時解析（live）'}
              </PickerItem>
            ))}
          </Picker>
        </div>
      ...
      {trace.result && (
        <section className={col} aria-label="模擬結果">
          <Kv
            items={[
          ...
          {trace.result.evaluations.map((evaluation, i) => (
            <section className={card} key={i}>
              ...
              <TableView aria-label={'規則評估 ' + (i + 1)} styles={style({height: 400})}>
                <TableHeader>
                  <Column id="id" isRowHeader width={90}>
                    規則 ID
                  </Column>
                  <Column id="expression">運算式</Column>
                  <Column id="result" width={180}>
                    結果
                  </Column>
                  <Column id="missing" width={150}>
                    缺少輸入
                  </Column>
                </TableHeader>
                <TableBody items={evaluation.rules}>
                  {rule => (
                    <Row id={rule.rule_id}>
                      <Cell>{rule.rule_id}</Cell>
                      <Cell>
                        <span className={code}>{rule.expression ?? '—'}</span>
                      </Cell>
                      <Cell>
                        <span style={{opacity: rule.result === 'skipped' ? 0.5 : 1}}>
                          <StatusLight size="S" variant={rule.result === 'matched' ? 'positive' : rule.result === 'indeterminate' ? 'notice' : 'neutral'}>
                            <Text>{rule.result}</Text>
                          </StatusLight>
                        </span>
                      </Cell>
          ...
          {trace.result.dns.map(dns => (
            <section className={card} key={dns.lookup_id}>
          ...
          <div className={split}>
            <div className={col}>
              <TableView aria-label="配置規則" selectionMode="single" selectedKeys={sel} onSelectionChange={setSel} styles={style({height: 442})}>
                <TableHeader>
                  <Column id="n" width={48}>
                    #
                  </Column>
                  <Column id="c" isRowHeader>
                    條件
                  </Column>
                  <Column id="o" width={88}>
                    目標
                  </Column>
                  <Column id="m" width={80}>
                    must
                  </Column>
                  <Column id="s" width={130}>
                    來源
                  </Column>
                  <Column id="x" width={80}>
                    註解
                  </Column>
                </TableHeader>
                <TableBody items={config.rules}>
                  {r => (
                    <Row id={r.id}>
                      <Cell>{r.n}</Cell>
                      <Cell textValue={r.cond}>
                        <span className={code}>{r.cond}</span>
                      </Cell>
                      <Cell>{r.target}</Cell>
                      <Cell>{r.must ? 'must' : ''}</Cell>
                      <Cell>{r.generated ? '生成' : r.source}</Cell>
                      <Cell>{r.note}</Cell>
                    </Row>
                  )}
                </TableBody>
              ...
              <div className={fb}>
                <span className={kw}>fallback</span>: <span className={out}>{config.fallback.target}</span>
                <span className={label}>{config.fallback.source}，請在來源編輯器修改</span>
              </div>
            </div>
            <div className={card}>
              {cur ? (
                <>
                  ...
                  <div className={col}>
                    <ButtonGroup>
                      ...
                      <DialogTrigger>
                        <Button variant="negative" fillStyle="outline" isDisabled={!cur.editable}>
                          刪除
                        </Button>
                        <AlertDialog
                          variant="destructive"
                          title={'刪除規則 #' + cur.n}
                          primaryActionLabel="刪除並 reload"
                          cancelLabel="取消"
                          onPrimaryAction={() => toast('positive', '已刪除 #' + cur.n + '，reload 完成（r' + (runtime.diskRevision + 1) + '）')}
                        >
