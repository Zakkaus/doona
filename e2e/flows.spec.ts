...
test('retained-flow graph filters rows and clears with the chip, repeated click, or Enter', async ({page}) => {
  await page.goto('/#/flows');
  const chart = page.getByRole('region', {name: 'Flow distribution'});
  await expect(chart.locator('svg.recharts-surface')).toBeVisible();
  const nodes = chart.locator('.rp-sankey-node');
  expect(await nodes.count()).toBeGreaterThanOrEqual(4);
  const rows = page.locator('.rp-table tbody tr[data-key]');
  const total = await rows.count();
  const rule = nodes.filter({has: page.locator('title', {hasText: 'domain(full: api.telegram.org)'})});
  const firstRule = chart.locator('.rp-sankey-node[data-stage="rule"]').first();
  ...
  for (const label of ['Unknown source', 'Unknown rule', 'Unknown chain', 'Unknown outbound']) {
    await expect(chart.locator('.rp-sankey-label').getByText(label, {exact: true})).toBeVisible();
  }
});
