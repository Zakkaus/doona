...
test('a custom icon mapping shows on the group and can be removed', async ({page}) => {
  ...
  await card.getByLabel('Icon', {exact: true}).fill('netflix');
  await card.getByRole('button', {name: 'Add', exact: true}).click();
  await expect(card.locator('img.brand')).toHaveAttribute('src', /brands\/netflix\.png$/);
  await page.goto('/#/policies');
  const title = page.locator('.rp-content .rp-card').first().locator('.rp-row').first();
  await expect(title.locator('img.brand')).toHaveAttribute('src', /brands\/netflix\.png$/);
  await page.goto('/#/settings');
