...
test('profiles add, rename, save on switch, and delete without losing the route', async ({page}) => {
  ...
  await expect(page.locator('.rp-side-backend')).toHaveCount(0);
  ...
  await expect(page.locator('.rp-side-backend')).toContainText('Office');
  ...
  await Promise.all([page.waitForEvent('load'), dialog.getByRole('button', {name: 'Save', exact: true}).click()]);
  await expect(page.locator('.rp-side-backend')).toContainText('Home router');
