...
test('a hidden cache tab stops walking the cache until it is shown again', async ({page}) => {
  ...
  const walks = () => requests.filter(request => request.method() === 'GET' && new URL(request.url()).pathname === '/api/v1/dns/cache').length;
  ...
  // Query has no cache consumer; Statistics legitimately reads cache usage even with the listing hidden.
